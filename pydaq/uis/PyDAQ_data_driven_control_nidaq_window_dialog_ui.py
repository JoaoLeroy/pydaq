...
def ds(parent,minimum,maximum,value,dec=6,suffix=""):
    w=QDoubleSpinBox(parent);w.setDecimals(dec);w.setRange(minimum,maximum);w.setValue(value);w.setSuffix(suffix);return w

def spin(parent,minimum,maximum,value):
    w=QSpinBox(parent);w.setRange(minimum,maximum);w.setValue(value);return w
...
class Ui_Dialog_Plot_Data_Driven_NIDAQ_Window(object):
    def setupUi(self,dialog):
        ...
        for label,w in [('Sample',self.label_collection_sample),('Applied input',self.label_collection_u),('Output y',self.label_collection_y),('Last cycle',self.label_cycle_time)]:fi.addRow(label,w)
        ...
        # NCbT/Arduino tab
        self.tab_ncbt=QWidget();self.tabs.addTab(self.tab_ncbt,'NCbT & NI-DAQ');mid=QHBoxLayout(self.tab_ncbt)
        ...
        fn.addRow('l',self.spin_l);fn.addRow('num_M',self.edit_num_M);fn.addRow('den_M',self.edit_den_M);fn.addRow('β',self.edit_beta);col2.addWidget(self.group_ncbt)
        ...
        fr.addRow('rho',self.edit_rho);fr.addRow('Samples',self.label_samples_used);fr.addRow('Estimation time (s)',self.label_estimation_time);fr.addRow('Status',self.label_design_status);col2.addWidget(self.group_result)
        ...
        for label,w in [('Sample',self.label_control_sample),('Output y',self.label_current_y),('Input u',self.label_current_u),('Unconstrained u',self.label_raw_u),('Saturated',self.label_saturation),('Overruns',self.label_overruns)]:fic.addRow(label,w)
