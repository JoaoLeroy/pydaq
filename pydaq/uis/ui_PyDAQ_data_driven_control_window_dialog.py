...
class Ui_Dialog_Plot_Data_Driven_Window(object):
    def setupUi(self,dialog):
        dialog.setObjectName('Dialog_Plot_Data_Driven_Window');dialog.resize(1180,820);dialog.setMinimumSize(QSize(980,680));dialog.setStyleSheet(STYLE)
        ...
        col1=QVBoxLayout();mid.addLayout(col1,1);self.group_arduino=QGroupBox('Arduino hardware');fa=QFormLayout(self.group_arduino)
        serial_row=QHBoxLayout();self.combo_serial=QComboBox();self.button_refresh=QPushButton('Refresh');serial_row.addWidget(self.combo_serial,1);serial_row.addWidget(self.button_refresh)
        fa.addRow('Device',serial_row);self.combo_ai=QComboBox();self.combo_ai.addItems([f'A{i}' for i in range(6)]);self.combo_pwm=QComboBox();self.combo_pwm.addItems(['D3','D5','D6','D9','D10','D11'])
        for label,w in [('AI channels',self.combo_ai),('AO channels',self.combo_pwm)]:fa.addRow(label,w)
        ...
        self.group_result=QGroupBox('Estimated controller')
        fr=QFormLayout(self.group_result)
        fr.setFieldGrowthPolicy(QFormLayout.FieldGrowthPolicy.AllNonFixedFieldsGrow)
        fr.setHorizontalSpacing(12)
        fr.setVerticalSpacing(7)

        # rho is a one-line vector result. A QPlainTextEdit created an internal
        # vertical scrollbar, so the global PyDAQ scrollbar arrows appeared at
        # the right and made this field look like a numeric spin box.
        self.edit_rho=QLineEdit(self.group_result)
        self.edit_rho.setObjectName('edit_rho')
        self.edit_rho.setReadOnly(True)
        self.edit_rho.setPlaceholderText('Controller not estimated')
        self.edit_rho.setMinimumHeight(25)
        self.edit_rho.setStyleSheet(
            'QLineEdit#edit_rho {'
        ...
        self.label_samples_used=QLabel('0',self.group_result)
        self.label_estimation_time=QLabel('0.00000 s',self.group_result)
        self.label_design_status=QLabel('Not estimated',self.group_result)
        for value_label in (
            self.label_samples_used,
            self.label_estimation_time,
            self.label_design_status,
        ):
            value_label.setMinimumHeight(25)
            value_label.setAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)

        fr.addRow('ρ',self.edit_rho)
        fr.addRow('Samples',self.label_samples_used)
        fr.addRow('Estimation time (s)',self.label_estimation_time)
        fr.addRow('Status',self.label_design_status)
        col2.addWidget(self.group_result)
